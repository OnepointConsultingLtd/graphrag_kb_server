...
function JokerButton() {
  ...
  const { topics, setInputText } = useChatStore(
    useShallow((state) => ({
      topics: state.topics,
  ...
  return (
    <button className="w-11 h-11 -mt-1 cursor-pointer filter hue-rotate-120 saturate-200 brightness-155 tooltip" data-tip="Click for random topic" onClick={handleClick}>
...
export function TopicSwitcher() {
  ...
  return (
    <div className="flex flex-row gap-2 justify-end">
      <button className="w-5 h-5 tooltip filter brightness-150 cursor-pointer" data-tip={showTopics ? "Hide topics" : "Show topics"} onClick={() => setShowTopics(!showTopics)}>
