...
def generate_direct_url(
    ...
    match chat_type:
        ...
        case WidgetType.CHAT:
            path = "chat"
    return f"{cfg.server_base_url}/{path}?project={project.name}&platform={project.platform}&search_type={project.search_type}&token={jwt_token}&chat_type={path}&streaming={"true" if streaming else "false"}"
