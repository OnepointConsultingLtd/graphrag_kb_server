...
from graphrag_kb_server.service.db.db_persistence_expanded_entities import create_expanded_entities_table, drop_expanded_entities_table, insert_expanded_entities, get_expanded_entities
from graphrag_kb_server.test.model.match_query_provider import create_match_query, create_match_output
...
async def test_create_expanded_entities():
    ...
    async def test_function(full_project: FullProject, _: Project, schema_name: str, project_name: str):
        ...
        try:
            ...
            digest = await insert_expanded_entities(fake_project_dir, match_query, match_output)
            assert digest is not None
            mached_output_from_db = await get_expanded_entities(fake_project_dir, match_query)
